...
const toolExampleMap: Record<string, (exampleNumber: number, focusChainSettings: FocusChainSettings) => string> = {
 ...
 */
export function createExampleSection(focusChainSettings: FocusChainSettings): string {
	const examples = [
		getExecuteCommandExample(1, focusChainSettings),
		getWriteFileExample(2, focusChainSettings),
		getNewTaskExample(3),
		getReplaceInFileExample(4, focusChainSettings),
	...
	const examples = sortedTools
		.map((toolName) => {
			try {
				...
				const example = generator(currentExampleNumber, focusChainSettings)
