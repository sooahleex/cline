...
export const SYSTEM_PROMPT = async (
	...
	const shouldUseModularSystem = useModularSystem ?? process.env[MODULAR_SYSTEM_ENV_VAR] === 'true';
	...
	if (shouldUseModularSystem) {
		try {
			...
			return await MODULAR_SYSTEM_PROMPT(cwd, supportsBrowserUse, mcpHub, browserSettings, isNextGenModel, customInstructions);
		} catch (error) {
...
- Before using the execute_command tool, you must first think about the SYSTEM INFORMATION context provided to understand the user's environment and tailor your commands to ensure they are compatible with their system. You must also consider if the command you need to run should be executed in a specific directory outside of the current working directory '${cwd.toPosix()}', and if so prepend with \`cd\`'ing into that directory then executing the command (as one command since you are stuck operating from '${cwd.toPosix()}'). For Windows PowerShell, use semicolon (;) to chain commands: \`cd (path to project); (command)\`. For Windows CMD, use && or &: \`cd (path to project) && (command)\`. For Unix/Linux/macOS, use &&: \`cd (path to project) && (command)\`. For example, if you needed to run \`npm install\` in a project outside of '${cwd.toPosix()}', you would need to prepend with a \`cd\` i.e. pseudocode for this would be \`cd (path to project) && (command, in this case npm install)\`.
