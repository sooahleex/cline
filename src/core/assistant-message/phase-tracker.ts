...
export class PhaseTracker {
  ...
  private phaseChangeListeners: ((phaseId: number, newStatus: PhaseStatus | 'in-progress' | 'completed' | 'skipped') => void)[] = [];
  ...
  private completePhase(phaseId: number, summary: string = '', thinking: string[] = []): void {
    ...
    phase.subtasks.forEach(st => {
      if (!st.completed) {st.completed = true};
  ...
  public async moveToNextPhase(contextSummary?: string): Promise<string | null> {
    ...
    const prompt = contextSummary
      ? [`# Previous Phase Summary:`, contextSummary, ``, `# Current Phase (${next.index}/${this.phases.length}):`, next.phase_prompt].join('\n')
      : next.phase_prompt;
    ...
    await this.controller.postMessageToWebview({type: 'invoke', invoke: 'sendMessage', text: prompt});
  ...
  private async executeConditionally(): Promise<void> {
    ...
    for (const phase of this.phases) {
      const should = conditions[phase.id]
        ? await conditions[phase.id]()
  ...
  public get currentPhase(): Phase {
    ...
    return { index: p.index, phase_prompt: p.phase_prompt, paths: p.paths, thinking: p.thinking, status: p.status as PhaseStatus, subtasks: p.subtasks.map(s => ({ description: s.description, completed: s.completed, type: s.type })) };
  }
  ...
  public generatePhaseExecutionReport(): any {
    return { totalPhases: this.totalPhases, completedPhases: this.phaseResults.length, phaseDetails: this.phaseResults };
  }

  private notifyPhaseChange(id: number, status: PhaseStatus | 'in-progress' | 'completed' | 'skipped'): void {
    this.phaseChangeListeners.forEach(l => { try { l(id, status); } catch {} });
  }
